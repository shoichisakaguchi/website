...
原本が `~/Dropbox/omc/RdRp_Summit/RVJC/` にも残るので、そちらの扱いも同時に決めること
（YouTube から消しても Dropbox に残っていては約束を守ったことにならない）。

⚠ **Zenodo の提案には返事が無い。**Heli も Tatiana も録画の話だけに答えた。未決のまま。
断られたわけではないので、次の演者で改めて出せる。
